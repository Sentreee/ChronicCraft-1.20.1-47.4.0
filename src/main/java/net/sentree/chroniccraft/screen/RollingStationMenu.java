...
public class RollingStationMenu extends AbstractContainerMenu {
    ...
    public RollingStationMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(iItemHandler -> {
            this.addSlot(new SlotItemHandler(iItemHandler, 0, 70, 11));
            this.addSlot(new SlotItemHandler(iItemHandler, 1, 91, 11));

            this.addSlot(new SlotItemHandler(iItemHandler, 2, 80, 59) {
                @Override
                public boolean mayPlace(ItemStack stack) {
                    return false; // prevent placing any items into the output slot
                }

                @Override
                public boolean mayPickup(Player playerIn) {
                    return true; // allow taking items out (you can set this to false to block pickup too)
                }
            });
        });
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, ModBlocks.PACKING_STATION.get());
    }
